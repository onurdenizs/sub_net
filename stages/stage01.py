...
def merge_geo_shapes(geo_1: str, geo_2: str) -> dict:
    ...
    try:
        ...
        return {"type": "LineString", "coordinates": combined_coords}
...
def print_all_segments(segment_df: pd.DataFrame):
    ...
    for i in range(0, len(segment_df)):
        ...
        print(
            f"Segment no: {str(i+1)}   {current_START_OP} - {current_END_OP} {str(current_polygon_length)}"
        )
...
def choose_action(
    ...
    print(
        f"first_segment_check {first_segment_check} - last_segment_check {last_segment_check}"
    )
    # ------------------------
    ...
    # ------------------------
    if (
        not last_segment_check and first_segment_check
    ):  # This is the first segment, and this is NOT the last segment
        ...
        if (
            current_END_OP in NEVER_SKIP_LIST
            and current_START_OP not in NEVER_SKIP_LIST
        ):
    ...
    if (
        not last_segment_check and not first_segment_check
    ):  # This is NOT the first segment, and this is NOT the last segment
        ...
        previous_segment["START_OP"].values[0]
        previous_END_OP = previous_segment["END_OP"].values[0]
        if (
            current_START_OP not in NEVER_SKIP_LIST
            and current_END_OP not in NEVER_SKIP_LIST
            and next_START_OP not in NEVER_SKIP_LIST
            and next_END_OP not in NEVER_SKIP_LIST
        ):
    ...
    if (
        last_segment_check and not first_segment_check
    ):  # This is NOT the first segment, and this is the last segment
        ...
        if current_START_OP not in NEVER_SKIP_LIST:
            ...
            return segment_df, i
        if (
            current_START_OP in NEVER_SKIP_LIST
            and current_END_OP not in NEVER_SKIP_LIST
        ):
...
def remove_first_segment(segment_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        print(
            f"🗑️ REMOVE FIRST SEGMENT IS TRIGGERED for segment {current_START_OP} - {current_END_OP}"
        )
...
def remove_last_segment(segment_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        print(
            f"🗑️ REMOVE LAST SEGMENT IS TRIGGERED for segment {last_START_OP} - {last_END_OP}"
        )
...
def combine_next_segment(segment_df: pd.DataFrame, i: int) -> tuple[pd.DataFrame, int]:
    try:
        ...
        print(
            f"✅ combine_next_segment IS TRIGGERED for: {current_START_OP} - {current_END_OP} + {next_START_OP} - {next_END_OP}"
        )

        # Merge geo shapes
        merged_geo_shape = merge_geo_shapes(
            current_segment["Geo shape"], next_segment["Geo shape"]
        )
        merged_coordinates = parse_geo_shape(merged_geo_shape)
        combined_length = (
            current_segment["polygon_length"] + next_segment["polygon_length"]
        )
        ...
        # Delete old rows
        segment_df = segment_df.drop(segment_df.index[[i, i + 1]]).reset_index(
            drop=True
        )
...
def combine_previous_segment(
    segment_df: pd.DataFrame, i: int
) -> tuple[pd.DataFrame, int]:
    try:
        ...
        print(
            f"✅ combine_previous_segment IS TRIGGERED for: {previous_START_OP} - {previous_END_OP} + {current_START_OP} - {current_END_OP}"
        )

        # Merge geo shapes
        merged_geo_shape = merge_geo_shapes(
            previous_segment["Geo shape"], current_segment["Geo shape"]
        )
        merged_coordinates = parse_geo_shape(merged_geo_shape)
        combined_length = (
            previous_segment["polygon_length"] + current_segment["polygon_length"]
        )
        ...
        # Delete two old rows
        segment_df = segment_df.drop(segment_df.index[[i - 1, i]]).reset_index(
            drop=True
        )
...
def run():
    ...
    print("Final Version of segments")
    print_all_segments(segment_df)
