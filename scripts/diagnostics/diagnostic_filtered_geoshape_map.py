...
color_map = {
    line_id: color_list[i % len(color_list)] for i, line_id in enumerate(line_ids)
}
...
for idx, row in df.iterrows():
    ...
    # Çiz
    folium.PolyLine(
        coords_wgs, color=color, weight=3, tooltip=f"{start_op} - {end_op} ({line_id})"
    ).add_to(m)

    # Start ve end noktalarına O işareti
    folium.Marker(
        coords_wgs[0],
        icon=features.DivIcon(
            icon_size=(150, 36),
            icon_anchor=(7, 20),
